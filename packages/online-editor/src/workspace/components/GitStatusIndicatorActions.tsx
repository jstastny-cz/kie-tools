...
import { FileStageStatus, FileStageStatusType } from "@kie-tools-core/workspaces-git-fs/dist/services/GitService";
...
export const GitStatusIndicatorActions = (props: GitStatusIndicatorActionsProps) => {
  ...
  const stageStatusFilter = React.useCallback(
    (applicableFor: FileStageStatus[]) => {
      return (fileStageStatus: FileStageStatusType) => {
        return (
          applicableFor.includes(fileStageStatus.status) &&
          (props.workspaceFile
            ? [props.workspaceFile.relativePath]
            : props.workspaceGitStatusPromise!.data!.fileStageStatuses.map(({ path }) => path)
          ).includes(fileStageStatus.path)
        );
      };
    },
    [props.workspaceFile, props.workspaceGitStatusPromise]
  );
  ...
  const doRevert = React.useCallback(
    async (filepaths: string[]) => {
      await workspaces.checkoutFilesFromLocalHead({
        ...
        ref: props.workspaceDescriptor.origin.branch,
        filepaths,
      });
      if (
  ...
  const actionRevert = React.useCallback((): GitStatusIndicatorActionType | undefined => {
    const maybeWorkspaceGitStatus = props.workspaceGitStatusPromise?.data;
    if (!maybeWorkspaceGitStatus) {
      return;
    }
    const id = props.workspaceFile ? SupportedActions.revert : SupportedActions.revertAll;

    const filepaths = maybeWorkspaceGitStatus.fileStageStatuses
      .filter(stageStatusFilter([FileStageStatus.deleted, FileStageStatus.modified, FileStageStatus.added]))
      .map((it) => it.path);
